      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-6">
            {Object.entries(categories).map(([key, label]) => (<TabsTrigger key={key} value={key}>
                {label}
              </TabsTrigger>))}
            <TabsTrigger value="custom">직접 추가</TabsTrigger>
          </TabsList>

          {Object.keys(categories).map((category) => (<TabsContent key={category} value={category} className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                ...
                return (<Card key={preset.id} className="cursor-pointer border-2 p-4 transition-all hover:border-primary hover:shadow-lg" onClick={() => {
                        onAddWidget(preset);
                        setOpen(false);
          ...
          <TabsContent value="custom" className="space-y-4">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="custom-name" placeholder="예: 내 증권사" value={customName} onChange={(e) => setCustomName(e.target.value)}/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="custom-url">웹사이트 URL</Label>
                <Input id="custom-url" placeholder="https://example.com" value={customUrl} onChange={(e) => setCustomUrl(e.target.value)}/>
              </div>
