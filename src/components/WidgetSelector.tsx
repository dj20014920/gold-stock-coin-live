...
interface WidgetSelectorProps {
  ...
  onAddCustomWidget: (url: string, name: string) => void;
}

export const WidgetSelector = ({ onAddWidget, onAddCustomWidget }: WidgetSelectorProps) => {
          ...
          {Object.keys(categories).map((category) => (
            <TabsContent key={category} value={category} className="space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                {WIDGET_PRESETS.filter(
                  (preset) => category === "all" || preset.category === category
                ).map((preset) => {
                  const IconComponent = (LucideIcons as any)[preset.icon] || LucideIcons.Globe;
                  return (
                    <Card
                      key={preset.id}
                      className="cursor-pointer border-2 p-4 transition-all hover:border-primary hover:shadow-lg"
                      onClick={() => {
                        onAddWidget(preset);
                        setOpen(false);
